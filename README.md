...
[Add your own IP](custom_ip/) · [Integration walkthrough](doc/tutorials/cnn_accel_integration_walkthrough.md)

## Extending Coral NPU — add your own IP

Adding a compute accelerator (CNN / MatMul / GEMM engine)? The [`custom_ip/`](custom_ip/)
workspace is a complete worked example — the `CnnAccel` int8 engine integrated as a
memory-mapped **bus device (MMIO + DMA)**, in an ASIC-style layout you can copy:

- **[`custom_ip/01_RTL/`](custom_ip/01_RTL/)** — the IP design (`CnnAccel.sv`).
- **[`custom_ip/00_TB/`](custom_ip/00_TB/)** — SystemVerilog testbenches (unit + whole-chip)
  and a VCS `run.sh`.
- **[`custom_ip/build_coralnpu.sh`](custom_ip/build_coralnpu.sh)** — builds the NPU and emits
  the whole-chip SystemVerilog.

Step-by-step on **which files to change and how** to wire an engine into the SoC (crossbar
map, the Chisel BlackBox shim, the load-bearing match arm), then test it:
